...
## Counting words and proportions by gender
...
props <- text %>%
  ...
  mutate(abs.dif.p = abs(proportion_F-proportion_M),
         ...
         abs.dif.n = abs(n_F-n_M)) %>%
  mutate(label.p= ifelse(abs.dif.p>=0.003,word,NA),
         label.n= ifelse(abs.dif.n>=4,word,NA))
```
...
ggplot(props, aes(x=proportion_M,, y=proportion_F,
 ...
 # geom_jitter(alpha = 0.5, size = 2.5, width = 0.3, height = 0.3) +
  ...
  scale_x_log10(name="Most used words Males",
                labels = percent_format(), limits=c(0.0005,0.03)) +
  scale_y_log10(name="Most used words Female",
                labels = percent_format(),limits=c(0.0005,0.03)) +
...
Words that are close to the line in these plots have similar frequencies in both genders. Words that are far from the line are words that are found more in one set of texts than another.
...
```{r}
props2 <- text %>% filter(word %in% props$label.p) %>%
  count(gender, word) %>%
  group_by(gender) %>%
  mutate(proportion = n / sum(n)) %>% 
  pivot_wider(names_from = gender, values_from = c(proportion,n))

test <- props2 %>% filter(!is.na(label.p)) %>%
...
ggplot(aes(x=proportion, y=word,fill=gender)) +
  ...
  scale_fill_manual(name="gender", values=c("#6D57CF","#FCA532"),
   ...
   geom_vline(xintercept = c(-0.02,-0.01,0,0.01,0.02),
              linetype="dotted",
             col="darkgray") +
  scale_x_continuous(breaks=c(-0.02,0,0.02),limits=c(-0.025,0.025),
...
```{r}
ggplot(props, aes(x=n_M, y=n_F,
       color=abs.dif.n)) + 
  geom_abline(color = "gray40", lty = 2) +
  geom_point(size=2.5, alpha=0.5)+
  geom_text_repel(aes(label=label.n))+
  scale_x_log10() +
  scale_y_log10() +
  scale_color_gradient(name="Abs Diff",low = "blue", high = "red") 
```
...
## ONLY PROFESSORS: Counting words and proportions by gender 
...
```{r}
props <- text %>% filter(position_cat == "professor") %>%
  count(gender, word) %>%
  #filter(n>1) %>% # removendo word ditas apenas uma vez
  group_by(gender) %>%
  mutate(proportion = n / sum(n)) %>% 
...
ggsave("figures/title_wordFrequency_PROF.jpeg", units="in", width=7, height=7, dpi=300)
```
...
```{r}
data <- read.table("data/presentations_PPGE_2008-2019_v2.csv", sep=",",
                   header=T, as.is=T)
data$date <- dmy(data$date)
data$year <- year(data$date) 
#skimr::skim(data)
